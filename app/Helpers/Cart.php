...
class Cart
{
    public static function add($product_id, $qty){
        ...
        if (Session::has('cart')){
            ...
            if(!$set){
                $cart['amount'] += $qty * $product->discounted_price;
                $cart['qty'] += $qty ;
                array_push($cart['items'],['id' => $product_id, 'qty' => $qty]);
            }
            else{
                if($qty < $cart['items'][$index]['qty']){
                    $cart['amount'] -= ($cart['items'][$index]['qty'] - $qty) * $product->discounted_price;
                    $cart['qty'] -= $cart['items'][$index]['qty'] - $qty;
                }else if ($qty > $cart['items'][$index]['qty']){
                    $cart['amount'] += ($qty - $cart['items'][$index]['qty']) * $product->discounted_price;
    ...
    public static function increase($productId){
        ...
        $cart['amount'] += $product->discounted_price;
        $cart['qty'] ++;
    ...
    public static function remove($product_id){
        ...
        $cart['amount'] -= $product->discounted_price * $cart['items'][$index]['qty'];
    ...
    public static function update($products){
        ...
        foreach($cart['items'] as $key =>  $item){
            ...
            $totalamount += $products[$item['id']] * $product->discounted_price;
        }
